...
CREATE OR REPLACE FUNCTION obtContratoRedeterminado (VidObra NUMERIC(5))
...
BEGIN
	...
	COALESCE((Select sum((((ic.costo*(1+o.porflete))*(1+o.porgastos))*(1+o.poruti))*(1+o.ivainfra)) as costoinfra from obra o
        ...
        inner join redeterminacion r on r.idredeterminacion = ic.idredeterminacion
        where (r.fechahasta is null) and (i.idtipoitem = 2) and (o.idobra = VidObra))
    ...
    (Select sum((((ic.costo*(1+(o.porflete/100)))*(1+(o.porgastos/100)))*(1+(o.poruti/100)))*(1+(o.ivaviv/100))) as costoviv from obra o
        ...
        inner join itemcosto ic on (ic.iditem = i.iditem) and (ic.idobra = i.idobra)
        inner join redeterminacion r on r.idredeterminacion = ic.idredeterminacion
        where (r.fechahasta is null) and (i.idtipoitem = 1) and (o.idobra = VidObra)) as tablaviv;
	-----------------------------------------------------
RETURN CCalculado;
END;
$$ LANGUAGE plpgsql